...
class FFmpegStreamPlayer {
  ...
  constructor(audioContext, workletPath = null) {
    ...
    // Session tracking to prevent stale worklet messages
    this.workletSessionId = 0;
  }
  ...
  async open(filePath, workletUrl = null) {
    ...
    // Increment session ID for new worklet
    this.workletSessionId++;
    const currentSessionId = this.workletSessionId;
    ...
    this.workletNode.port.onmessage = (event) => {
      // Ignore messages from old worklet sessions
      if (currentSessionId !== this.workletSessionId) {
        return;
      }
